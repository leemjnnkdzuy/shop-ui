...
import styles from "./AddVgaBody.module.scss";
...
const AddVgaBody = () => {
	...
	const handleSubmit = async (e) => {
		...
		try {
			const response = await request.post("api/vgaItem", data, {
				headers: {
			...
			if (response.status === 201) {
				setSuccessMessage("VGA added successfully");
				setShowPopup(true);
				clearForm();
		...
		<div className={cx("wrapper")}>
			<form onSubmit={handleSubmit}>
				...
				<input
					...
					onChange={handleChange}
				/>
				<button type="submit">Add VGA</button>
			</form>
