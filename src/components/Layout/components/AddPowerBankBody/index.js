...
import styles from "./AddPowerBankBody.module.scss";
import Popup from "~/components/Layout/components/Popup";

const cx = classNames.bind(styles);

const AddPowerBankBody = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        for (const key in formData) {
            ...
            } else if (["Color", "SafetyFeatures"].includes(key)) {
        ...
        try {
            const response = await request.post("api/powerBankItem", data, {
                headers: {
            ...
            if (response.status === 201) {
                setSuccessMessage("Power Bank added successfully");
                setShowPopup(true);
                clearForm();
            ...
            console.error("Error adding power bank:", error);
            const message =
                error.response?.status === 413
                    ? "Files are too large. Please reduce file sizes."
                    : error.response?.status === 400
                    ? "Please check all required fields and file formats."
                    : error.response?.data?.message || "An error occurred while adding the power bank";
    ...
    return (
        <div className={cx("wrapper")}>
            <form onSubmit={handleSubmit}>
                ...
                <input type="number" name="Capacity" placeholder="Capacity (mAh)" step="any" onChange={handleChange} />
                <input type="number" name="OutputPorts" placeholder="Number of Output Ports" onChange={handleChange} />
                <input type="number" name="InputPorts" placeholder="Number of Input Ports" onChange={handleChange} />
                <input type="text" name="OutputPower" placeholder="Output Power" onChange={handleChange} />
                <input type="text" name="InputPower" placeholder="Input Power" onChange={handleChange} />
                <input type="text" name="Size" placeholder="Size" onChange={handleChange} />
                <input type="number" name="Weight" placeholder="Weight (g)" step="any" onChange={handleChange} />
                <input type="text" name="BatteryType" placeholder="Battery Type" onChange={handleChange} />
                <input type="text" name="FastChargingSupport" placeholder="Fast Charging Support" onChange={handleChange} />
                <input type="text" name="SafetyFeatures" placeholder="Safety Features (comma-separated)" onChange={handleChange} />
                <input type="text" name="LEDIndicator" placeholder="LED Indicator" onChange={handleChange} />
                ...
                <button type="submit">Add Power Bank</button>
            </form>
...
export default AddPowerBankBody;
