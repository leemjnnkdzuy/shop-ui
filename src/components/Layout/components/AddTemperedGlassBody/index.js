...
import styles from "./AddTemperedGlassBody.module.scss";
import Popup from "~/components/Layout/components/Popup";

const cx = classNames.bind(styles);

const AddTemperedGlassBody = () => {
	...
	const handleSubmit = async (e) => {
		...
		try {
			const response = await request.post("api/temperedGlassItem", data, {
				headers: {
			...
			if (response.status === 201) {
				setSuccessMessage("Tempered glass added successfully");
				setShowPopup(true);
				clearForm();
			...
			console.error("Error adding tempered glass:", error);
			const message =
				error.response?.status === 413
					...
					: error.response?.data?.message ||
					  "An error occurred while adding the tempered glass";
	...
	return (
		<div className={cx("wrapper")}>
			<form onSubmit={handleSubmit}>
				...
				<input
					type="text"
					name="OleophobicCoating"
					placeholder="Oleophobic Coating"
					onChange={handleChange}
				/>
				<input
					type="text"
				...
				<input
					...
					onChange={handleChange}
				/>
				<button type="submit">Add Tempered Glass</button>
			</form>
...
export default AddTemperedGlassBody;
