...
import styles from "./AddHeadphoneBody.module.scss";
import Popup from "~/components/Layout/components/Popup";

const cx = classNames.bind(styles);

const AddHeadphoneBody = () => {
	...
	const handleSubmit = async (e) => {
		...
		try {
			const response = await request.post("api/headphoneItem", data, {
				headers: {
			...
			if (response.status === 201) {
				setSuccessMessage("Headphone added successfully");
				setShowPopup(true);
				clearForm();
		...
		<div className={cx("wrapper")}>
			<form onSubmit={handleSubmit}>
				...
				<input
					type="text"
					name="EarCupMaterial"
					placeholder="Ear Cup Material"
					onChange={handleChange}
				/>
				<input
					type="text"
					name="AdjustableHeadband"
					placeholder="Adjustable Headband"
					onChange={handleChange}
				/>
				<input
					type="text"
...
export default AddHeadphoneBody;
