...
function Header() {
	...
	const handleInputChange = (event) => {
		...
		const newTimeout = setTimeout(async () => {
			try {
				const response = await request.get(`api/homeItem/search?query=${value}`);
	...
	const handleSearch = async () => {
		...
		try {
			const response = await request.get(`api/homeItem/search?query=${inputValue}`);
	...
	return (
		<header className={cx("wrapper")}>
			<div className={cx("inner")}>
				...
				<Tippy
					interactive={true}
					visible={showResults && inputValue.trim().length > 0}
								...
								<div className={cx("search-list")}>
									{isSearching ? (
										<div className={cx("search-loading")}>
											...
											<div className={cx("search-results-title")}>Đang tìm kiếm...</div>
										</div>
										...
										<div className={cx("search-no-results")}>
											<img src={gifs.gif2} alt="No result" loop="true" />
											<div className={cx("search-results-title")}>
												Bọn mình không tìm thấy sản phầm bạn yêu cầu
											</div>
										</div>
									)}
								</div>
