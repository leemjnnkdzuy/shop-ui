...
import styles from "./AddHeatsinkBody.module.scss";
import Popup from "~/components/Layout/components/Popup";

const cx = classNames.bind(styles);

const AddHeatsinkBody = () => {
	...
	const handleSubmit = async (e) => {
		...
		try {
			const response = await request.post("api/heatsinkItem", data, {
				headers: {
			...
			if (response.status === 201) {
				setSuccessMessage("Heatsink added successfully");
				setShowPopup(true);
				clearForm();
		...
		<div className={cx("wrapper")}>
			<form onSubmit={handleSubmit}>
				...
				<input
					...
					placeholder="Cooling Performance"
					onChange={handleChange}
				/>
				<input
					type="text"
				...
				<input
					...
					onChange={handleChange}
				/>
				<button type="submit">Add Heatsink</button>
			</form>
...
export default AddHeatsinkBody;
